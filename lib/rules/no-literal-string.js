...
module.exports = {
  ...
  create(context) {
    ...
    const translationsFilePath = path.resolve(process.cwd(), options.translations)
    ...
    function addDefaultTranslation(text) {
      const translations = JSON.parse(fs.readFileSync(translationsFilePath, 'utf8'))
      // to upper snake case
      const key = text.replace(/\W+/g, " ").trim().split(/\s/).join('_').toUpperCase()
      if (!key || typeof translations[key] !== 'undefined') return
    ...
    function report(node) {
      context.report({
        ...
        fix: function(fixer) {
          const text = context.getSourceCode().getText(node).trim()
          if (!/^t\([A-Z_]*\)$/.test(text)) {
            const key = addDefaultTranslation(text)
            if (key) return fixer.replaceText(node, `t(${key})`)
          }
        },
      });
    }
